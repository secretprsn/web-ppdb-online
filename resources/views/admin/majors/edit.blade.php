...
@section('title', 'Edit Jurusan - ' . $major->nama_jurusan)
    ...
    <div class="card" style="padding: 1.5rem;">
        <form method="POST" action="{{ route('admin.majors.update', $major) }}">
            ...
            <div style="margin-bottom: 1.25rem;">
                ...
                <input type="text" name="nama_jurusan" id="nama_jurusan" class="form-input" value="{{ old('nama_jurusan', $major->nama_jurusan) }}" placeholder="Contoh: Rekayasa Perangkat Lunak" required>
                @error('nama_jurusan') <div class="form-error">{{ $message }}</div> @enderror
            </div>

            <div style="margin-bottom: 1.25rem;">
                <label for="kuota" class="form-label">Kuota Peserta</label>
                <input type="number" name="kuota" id="kuota" class="form-input" value="{{ old('kuota', $major->kuota) }}" placeholder="Jumlah kuota siswa baru" required min="1">
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4" class="form-input" placeholder="Jelaskan secara singkat tentang jurusan ini...">{{ old('deskripsi', $major->deskripsi) }}</textarea>
