    ...
    <div class="card" style="padding: 1.5rem;">
        <form method="POST" action="{{ route('admin.announcements.store') }}">
            ...
            <div style="margin-bottom: 1.25rem;">
                ...
                <input type="text" name="judul" id="judul" class="form-input" value="{{ old('judul') }}" placeholder="Judul besar pengumuman" required>
                @error('judul') <div class="form-error">{{ $message }}</div> @enderror
            </div>

            <div style="margin-bottom: 1.25rem;">
                <label for="tanggal_publish" class="form-label">Tanggal Publish</label>
                <input type="date" name="tanggal_publish" id="tanggal_publish" class="form-input" value="{{ old('tanggal_publish', date('Y-m-d')) }}" required>
                @error('tanggal_publish') <div class="form-error">{{ $message }}</div> @enderror
            </div>

            <div style="margin-bottom: 1.25rem;">
                <label for="isi" class="form-label">Isi Pengumuman</label>
                <textarea name="isi" id="isi" rows="10" class="form-input" placeholder="Tulis pengumuman di sini..." required>{{ old('isi') }}</textarea>
            ...
            <div style="margin-bottom: 1.5rem; display: flex; align-items: center; gap: 0.5rem;">
                ...
                <label for="is_published" class="form-label" style="margin-bottom: 0;">Publish Segera</label>
            </div>

            <div style="display: flex; gap: 0.75rem;">
                <button type="submit" class="btn-primary" style="padding: 0.75rem 2rem;">Buat Pengumuman</button>
